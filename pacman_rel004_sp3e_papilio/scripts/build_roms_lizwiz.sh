#!/bin/sh
# Converted to .sh by Glenn Neidermeier 29-April-2024
#
# http://forum.gadgetfactory.net/topic/1273-galaxian-alieninv-lizwiz/
# LizWiz uses a second block of BRAM to store its program code. It is the only Pacman based game that does so. 
# It required one too many BRAMs then available on the Papilio One/500K (Spartan-3E) so the ROM_PGM_1 block was
# commented out.
...
#REM concatenate consecutive ROM regions
cat $rom_path_src/wiza $rom_path_src/wizb > $rom_path/wiz.bin
cat $rom_path_src/5e.cpu $rom_path_src/5f.cpu > $rom_path/gfx1.bin
cat $rom_path_src/6e.cpu $rom_path_src/6f.cpu $rom_path_src/6h.cpu $rom_path_src/6j.cpu > $rom_path/main.bin
...
#REM this ROM area IS used AND required
$romgen_path/romgen $rom_path/wiz.bin           ROM_PGM_1 13 l r e > $rom_path/rom1.vhd
